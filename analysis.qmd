...
```{r}
#I wanted to see how complaints are groups across each borough
nycomplaints |>
  group_by(borough) |>
  summarize(total = n())

#I wanted to see how complaints are groups across each community board
nycomplaints |>
  group_by(community_board) |>
  summarize(total = n())

#I wanted to see plot of when complaints were opened for changes in data
nycomplaints |> 
  ggplot(aes(x = opendate)) + 
  geom_freqpoly(binwidth = 15)

#I wanted to see plot of when complaints were closed for changes in data
nycomplaints |> 
  filter(!is.na(closedate)) |>
  ggplot(aes(x = closedate)) + 
  geom_freqpoly(binwidth = 15)
```
...
```{r}
#I wanted to plot the different types of complaints over time, focusing on the top 5 complaints overall in the data
top_complaint_type <-
  nycomplainttype_top5 |>
  select(complaint_type)

ny_top_complaints <- nycomplaints |>
  filter(complaint_type %in% top_complaint_type$complaint_type)
...
ny_top_complaints |>
  ggplot(aes(x = opendate, color = complaint_type)) + 
  geom_freqpoly(binwidth = 30) +
  labs(title = "Top 5 Complaints From Constituents Over Time", subtitle = "NYC Council Office District Data", x = "Open Date", y = "Count", color = "Complaint Type")

#I wanted to see if the number of complaints with no closed date has changed over time
ny_not_closed <-
  nycomplaints |>
  filter(is.na(closedate))
...
ny_not_closed |>
  ggplot(aes(x = opendate)) + 
  geom_freqpoly(binwidth = 15) +
  labs(title = "Complaints With No Close Date Over Time", subtitle = "NYC Council Office District Data", x = "Open Date", y = "Count")
```
...
```{r}
monthly_counts <- ny_top_complaints %>%
  mutate(month_year = floor_date(opendate, "month")) %>%
  group_by(month_year, complaint_type) %>%
  summarise(count = n()) 

monthly_counts$complaint_type <- factor(monthly_counts$complaint_type, 
                      levels = c("Housing and Buildings", "Transportation", "Finance",
                                 "Immigration", "General Welfare"))

print(monthly_counts) 

ny_top_complaints_plot<- monthly_counts |>
  ggplot(aes(x = month_year, y = count, color = complaint_type)) + 
  geom_line() +
  labs(title = "Complaints From Constituents Over Time, 2015 - 2025", subtitle = "NYC Council Office District Data, Overall Top 5 Complaint Types", x = "Date Complaint Opened", y = "Count", color = "Complaint Type") +
scale_x_date(breaks = "1 year", minor_breaks = "1 month", date_labels = "%Y") +
      scale_color_manual(values = c("Housing and Buildings" = "#003f5c", "Transportation" = "#58508d", "Finance" ="#bc5090",  "Immigration" = "#ff6361",  "General Welfare" = "#ffa600")) +
  theme(text=element_text(family="serif"))

ny_top_complaints_plot
```
...
```{r}
ggsave(
  file = "topcomplaints.png",
  plot = ny_top_complaints_plot,
  device = png,
  path = "figures",
  scale = 1,
  width = 10,
